...
class TestScript(unittest.TestCase):
    ...
    def test_step(self):
        ...
        Common(DUT).clickById("callTo")
        Common(DUT).inputText("n1oXdmwuo5jJRExnZR5rbceMEyzRsPeALgm")
        Common(DUT).clickById("callValue")
        Common(DUT).inputText("0.000000000000000001")
        Common(DUT).clickById("callFunction")
        Common(DUT).inputText("save")
        Common(DUT).clickById("callArgs")
        Common(DUT).setText("callArgs",arg2)
        Common(DUT).clickByText("pay",index=2)
        Common(DUT).wait(2)
        ...
        result = Checkpoint(DUT).checkIfExist("检测点2:收款方", text="n1oXdmwuo5jJRExnZR5rbceMEyzRsPeALgm")
        self.assertEqual(result, True)
        Common(DUT).wait(40)
